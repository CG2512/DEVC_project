...
public class UserService implements UserDetailsService {
	...
	/*
	 * @Transactional public void update(UserDTO userDTO) { User currentUser =
	 * userRepo.findById(userDTO.getId()).orElseThrow(NoResultException::new);
	 * currentUser.setRoles(userDTO.getRoles().stream().map(role ->
	 * roleRepo.findById(role.getId()).orElse(null)) .filter(r -> r !=
	 * null).collect(Collectors.toList())); userRepo.save(currentUser); }
	 */
